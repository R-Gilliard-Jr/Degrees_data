...
Until now we have stored a single number, string, or TRUE/FALSE value in each object we created. But, if we were faced with a more complex problem, we could use vectors instead to solve it. Let's consider the task of figuring out who the tallest student is in a class. First, I'll generate some data. It is not important for you to know how to do this.
...
We could get the number of snacks eaten by each student by going throw the indices of the two vectors one-by-one and summing the values like so:
...
paste("The weather is", weather, "so I", umbrella, "bring my umbrella.")
```

The sentence changes depending on the weather. 

Here is an example with more applicability to data analysis. Suppose that you are conducting a study where some students are randomly assigned to attend private school. You want to compare the test scores of students assigned to private school and the test scores of students who remained in a public school. 
